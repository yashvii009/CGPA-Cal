...
class _CalculatorScreenState extends State<CalculatorScreen> {
  ...
  Widget buildCourseList() {
    return Expanded(
      child: ListView.builder(
        itemCount: semesters.isNotEmpty ? semesters.last.courses.length : 0,
        itemBuilder: (context, index) {
          return ListTile(
            title: Text(semesters.last.courses[index].title),
            subtitle: Text(
              '${semesters.last.courses[index].code} | '
              '${semesters.last.courses[index].credits} credits | '
              'Grade: ${semesters.last.courses[index].grade}',
            ),
          );
        },
      ),
  ...
  Widget buildStatistics() {
    ...
    final int totalCredits = calculateTotalCredits();
  ...
  void addSemester() {
    setState(() {
      semesters.add(
        Semester(
          name: semesterNameController.text,
          courses: [],
        ),
      );
    });
  ...
  double calculateCGPA() {
    // Implement CGPA calculation logic here
    return 0.0;
  }

  int calculateTotalCredits() {
    // Implement total credits calculation logic here
    return 0;
  }
}
